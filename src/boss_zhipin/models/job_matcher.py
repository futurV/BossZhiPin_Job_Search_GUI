...
# 匹配评分和招呼语生成不同：评分回复即使 HTTP 200，也可能没遵守“分数: 85”的
# 格式。因此不能只依赖 HTTP 层的重试；这里把“调用异常 / 解析失败”都算一次评分
# 尝试，最多向模型请求三次，最后才走既有 fail-open。
SCORE_MAX_ATTEMPTS = 3
SCORE_RETRY_BASE_DELAY_SECONDS = 0.5
...
- 0-49: 匹配度低，不建议投递"""
    ...
    for attempt in range(1, SCORE_MAX_ATTEMPTS + 1):
        # _call_chat_completion 仍保留它自己的网络退避；本循环额外处理模型虽然响应
        # 成功、但格式不合规的情形，重新请求一份可解析的评分。
        t0 = time.monotonic()
        try:
            response = _call_chat_completion(
                client,
            ...
            last_failure = f"调用异常：{type(e).__name__}: {e}"
            record_llm_call(
                provider=prov, model=llm_model,
                input_tokens=0, output_tokens=0,
                latency_ms=int((time.monotonic() - t0) * 1000),
                ok=False, error=last_failure,
            )
        else:
            content = _completion_content(response)
            ...
            # 冒号同时容忍 ASCII ":" 和全角 "："——中文 LLM（尤其 DeepSeek）即便
            # prompt 给的是 ASCII 冒号，回复也常用全角。
            ...
            if score_match:
                ...
                if attempt > 1:
                    log.info("LLM 评分第 %d/%d 次重试成功", attempt, SCORE_MAX_ATTEMPTS)
                return score, reason, False
            last_failure = f"回复解析失败：{content[:120]!r}"

        if attempt < SCORE_MAX_ATTEMPTS:
            delay = SCORE_RETRY_BASE_DELAY_SECONDS * attempt
            log.warning(
                "LLM 评分第 %d/%d 次失败（%s），%.1fs 后重试",
                attempt, SCORE_MAX_ATTEMPTS, last_failure, delay,
            )
            time.sleep(delay)

    log.warning(
        "LLM 评分连续 %d 次失败，按 100 放行：%s",
        SCORE_MAX_ATTEMPTS, last_failure,
    )
    return 100, f"评分失败，已重试 {SCORE_MAX_ATTEMPTS} 次（{last_failure}）", True
