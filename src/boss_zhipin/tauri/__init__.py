...
class RunConfig(_CamelModel):
    ...
    def validate_delay_range(self):
        if self.delay_max < self.delay_min:
            raise ValueError("delayMax 不能小于 delayMin")
        return self
...
@commands.command()
async def get_applications(body: _LimitBody) -> dict[str, object]:
    """读取成功投递 CSV，供 GUI 历史页展示公司/职位/JD 等完整记录。"""
    from boss_zhipin.gui.history import _applications_path, read_applications
    return {
        "applications": read_applications(limit=body.limit),
        "path": str(_applications_path().resolve()),
    }
