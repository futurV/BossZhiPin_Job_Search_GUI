...
class TestLlmMatchScore:
    ...
    def test_retries_unparseable_score_then_succeeds(self, monkeypatch, fake_client, telemetry_spy):
        replies = iter([
            _fake_response("这个岗位很适合"),
            _fake_response("分数：82\n理由：核心技能匹配"),
        ])
        sleeps: list[float] = []
        monkeypatch.setattr(job_matcher, "_call_chat_completion", lambda client, **kwargs: next(replies))
        monkeypatch.setattr(job_matcher.time, "sleep", lambda delay: sleeps.append(delay))

        score, reason, degraded = llm_match_score("JD", "简历", ["Python"])

        assert (score, reason, degraded) == (82, "核心技能匹配", False)
        assert sleeps == [job_matcher.SCORE_RETRY_BASE_DELAY_SECONDS]
        assert len(telemetry_spy) == 2

    def test_falls_open_only_after_three_score_attempts(self, monkeypatch, fake_client, telemetry_spy):
        calls = {"count": 0}
        monkeypatch.setattr(
            job_matcher,
            "_call_chat_completion",
            lambda client, **kwargs: calls.update(count=calls["count"] + 1) or _fake_response("无法判断"),
        )
        monkeypatch.setattr(job_matcher.time, "sleep", lambda delay: None)
