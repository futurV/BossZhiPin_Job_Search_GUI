...
def test_select_expectation_verifies_active_state(monkeypatch):
    async def scenario():
        states = iter(
            [
                {
        ...
        async def fake_click(label: str):
            assert label == "Python"
            return {"ok": True, "text": "Python(成都)"}

        async def fake_sleep(delay: float):
            return None

        monkeypatch.setattr(finding_jobs, "_expectation_state", fake_state)
        monkeypatch.setattr(finding_jobs, "_click_expectation", fake_click)
...
def test_select_expectation_does_not_fall_back_when_missing(monkeypatch):
    async def scenario():
        async def fake_state(label: str):
            return {
                "targetFound": False,
...
def test_dismiss_contact_confirmation_waits_until_overlay_is_gone(monkeypatch):
    async def scenario():
        results = iter(
            [
                {"ok": True, "method": "stay"},
                {"confirmationVisible": True, "realCards": 15},
                {"confirmationVisible": False, "realCards": 15},
            ]
        )
        ...
        monkeypatch.setattr(finding_jobs, "_safe_evaluate", fake_evaluate)
        monkeypatch.setattr(finding_jobs.asyncio, "sleep", fake_sleep)
        assert await dismiss_contact_confirmation() is True
