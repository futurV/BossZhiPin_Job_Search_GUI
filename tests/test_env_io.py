...
class TestReadEnv:
    ...
    def test_gui_auto_send_fields_round_trip(self, in_tmp_cwd, monkeypatch):
        updates = {
            "BOSS_AUTO_SEND_MAX_SENT": "8",
            "BOSS_AUTO_SEND_DELAY_MIN": "12",
            "BOSS_AUTO_SEND_DELAY_MAX": "45",
        }
        for key in updates:
            monkeypatch.delenv(key, raising=False)
        env_io.write_env(updates)
        current = env_io.read_env()
        assert {key: current[key] for key in updates} == updates
        assert all(os.getenv(key) == value for key, value in updates.items())
