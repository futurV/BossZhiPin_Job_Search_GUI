...
export default function RunPage() {
  ...
  async function handleStart() {
    ...
    if (
      !Number.isFinite(parsedDelayMin) || !Number.isFinite(parsedDelayMax) ||
      parsedDelayMin < 0 || parsedDelayMax < parsedDelayMin || parsedDelayMax > 3600
    ) {
      setStartError(t("run.errDelayRange"));
      return;
    }
    setBusy(true);
    clear();
      ...
      <section className="panel">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
          ...
          <Field label={t("run.fieldDryRun")}>
            ...
            <div
              className={[
                "mt-1 border-l-4 px-3 py-2 text-xs font-mono",
                dryRun
                  ? "border-[var(--ink)] bg-[var(--ink)] text-[var(--paper)]"
                  : "border-[var(--ink)] text-[var(--ink)]",
              ].join(" ")}
            >
              {dryRun ? t("run.modeDryRun") : t("run.modeLive")}
            </div>
          </Field>

          <Field label={t("run.fieldMaxSent")} hint={t("run.hintSuccessCount")}>
            <input
              type="number"
              min="1"
              max="100"
          ...
          <Field label={t("run.fieldDelay")} hint={t("run.hintSeconds")}>
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
              <input
                aria-label={t("run.fieldDelayMin")}
                type="number"
                min="0"
                max="3600"
                step="1"
                value={delayMin}
                onChange={(e) => setFormState({ formDelayMin: e.target.value })}
                disabled={running}
                className="field-input"
              />
              <span className="font-mono text-xs">—</span>
              <input
                aria-label={t("run.fieldDelayMax")}
                type="number"
                min="0"
                max="3600"
                step="1"
                value={delayMax}
                onChange={(e) => setFormState({ formDelayMax: e.target.value })}
                disabled={running}
                className="field-input"
              />
            </div>
...
const KIND_STYLES: Record<string, EventStyle> = {
  ...
  run_config: { variant: "default", prefix: "⚙" },
...
function EventRow({ ev }: { ev: ProgressEvent }) {
  ...
  const status = ev.payload.application_status;
  const reason = ev.payload.application_reason;
  const applicationStatus = status == null ? "" : String(status);
  const applicationReason = reason == null ? "" : String(reason);
  const visiblePayload = Object.entries(ev.payload).filter(
    ([k]) => k !== "application_status" && k !== "application_reason",
  );
  ...
  return (
    <div className={classes}>
      ...
      {applicationStatus && <span className="ml-2 font-bold">投递：{applicationStatus}</span>}
      {applicationReason && <span className="ml-1 opacity-80">（{applicationReason}）</span>}
      {payload && <span className="ml-2 opacity-80">{payload}</span>}
    </div>
  );
}
